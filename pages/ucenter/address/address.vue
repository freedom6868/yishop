<template>
	<view class="container">
		<view class="address-list" v-if="addressList.length > 0">
			<view class="item" v-for="item in addressList" :key='item.id' @click="addressAddOrUpdate"  :data-address-id='item.id' >
				<view class="l">
				...
				<view class="r">
					<image mode="" @click.stop ='deleteAddress'  :data-address-id="item.id" class="del" src="@/static/images/del-address.png"></image>
				</view>
			</view>
		</view>
...
<script>
	import {getAddressListData,addressDeleteData} from '@/api/uncenter/addressApi.js'
	export default {
		data() {
			return {
				addressList:[]
			...
			deleteAddress(e){
				...
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							let addressId =  e.currentTarget.dataset.addressId;
							let res2 = await addressDeleteData({id:addressId});
							console.log(res2)
							if (res2.errno === 0) {
